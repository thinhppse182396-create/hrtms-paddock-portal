  ...
  const publish = async (raceId: string) => {
    if (isBackendEnabled()) {
      try {
        ...
        toast.error("Cannot publish results", { description: error?.message });
        return;
      }
    }
        ...
        {loading ? (
          ...
          <>
            ...
            {grouped.map(g => {
              ...
              const canPublish = g.reportStatus === "Confirmed" && !allPublished;
              return (
                <div key={g.raceId} className="bg-card border border-border rounded-lg overflow-hidden">
                  <div className="flex items-center justify-between px-5 py-3 border-b border-border">
                    <div>
                      <div className="font-semibold text-foreground">{g.raceId} — {g.race?.track}</div>
                      <div className="text-xs text-muted-foreground">Referee report: <StatusBadge status={g.reportStatus} /></div>
                    </div>
                    <Button disabled={!canPublish} onClick={() => publish(g.raceId)}>
                      {allPublished ? "Published" : "Publish Result"}
                    </Button>
                  </div>
                  <DataTable
                    columns={[
