...
function AdminAwards() {
    ...
    const fetchAllAdminData = async () => {
      ...
      try {
        const [
          ...
          fetch("/api/admin/award-ceremonies").then(r => r.json()),
          ...
          fetch("/api/admin/jockeys").then(r => r.json()),
  ...
  const upsertCeremony = async (v: AwardCeremony) => {
    ...
    try {
      const url = isEdit ? `/api/admin/award-ceremonies/${editing.id}` : "/api/admin/award-ceremonies";
      ...
      setCeremonies(cs => isEdit ? cs.map(c => c.id === editing.id ? savedCeremony : c) : [...cs, savedCeremony]);
      setQ(""); setStatus("");
  ...
  const removeCeremony = async (c: AwardCeremony) => {
    try {
      const res = await fetch(`/api/admin/award-ceremonies/${c.id}`, { method: "DELETE" });
      if (!res.ok) throw new Error("API Error");

      setCeremonies(cs => cs.filter(x => x.id !== c.id));
      setDeleting(null);
