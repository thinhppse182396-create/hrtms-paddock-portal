...
export const Route = createFileRoute("/owner/my-horses")({
  component: MyHorses,
});
...
const fields: Field[] = [
  { name: "name", label: "Horse name", required: true, full: true },
  {
    ...
    type: "select",
    required: true,
    options: ["Thoroughbred", "Arabian", "Quarter Horse"].map((b) => ({
      label: b,
      value: b,
    })),
  },
  ...
  },
  {
    ...
    type: "select",
    required: true,
    options: ["Eligible", "Ineligible", "Suspended"].map((s) => ({
      label: s,
      value: s,
    })),
  },
...
function MyHorses() {
  ...
  const myHorses = useMemo(
    () => rows.filter((h) => h.ownerId === OWNER_ID),
    [rows]
  );
  ...
  const nextId = () => {
    const max = rows.reduce(
      (m, h) => Math.max(m, Number(h.id.replace(/\D/g, "")) || 0),
      0
    );
  ...
  const checkDuplicates = (v: any, ignoreId?: string) => {
    ...
    if (
      v.name &&
      others.some(
        (h) => h.name.trim().toLowerCase() === v.name.trim().toLowerCase()
      )
    ) {
    ...
    if (
      v.microchipId &&
      others.some((h) => h.microchipId === v.microchipId)
    ) {
  ...
  const addHorse = async (v: any) => {
    try {
      const newHorse = await createHorse({
        ...
        microchipId: `MC-${nextId()}`,
        documents: [
          {
            type: "Health Certificate",
            number: `HC-${nextId()}`,
            issuedBy: "National Equine Vet Board",
            issuedDate: new Date().toISOString().slice(0, 10),
            expiryDate: v.healthCertExpiry,
          },
        ],
      });

      setRows((prev) => [...prev, newHorse]);
      setCreating(false);

      toast.success("Horse added", {
        description: `${newHorse.name} (${newHorse.id})`,
      });
    } catch (err) {
      console.error(err);
    ...
    <div>
      ...
      <DataTable
        columns={[
          ...
          { key: "weight", header: "Weight (kg)" },
          {
            key: "docs",
            header: "Documents",
            render: (r) => `${r.documents.length} papers`,
          },
      ...
      <FormModal
        ...
        validate={(v: any) => {
          ...
          if (Number(v.age) < 2 || Number(v.age) > 20)
            e.age = "Age must be 2–20";
          if (Number(v.weight) < 400 || Number(v.weight) > 600)
            e.weight = "Weight must be 400–600kg";
