...
function RaceManagement() {
  ...
  const [confirmStarting, setConfirmStarting] = useState<Race | null>(null);
  const startRace = (race: Race) => {
    const updated = { ...race, status: "Ongoing" as Race["status"] };
    setRows(r => r.map(x => x.id === race.id ? updated : x));
    auditLog.add({ actor: "System Admin", action: "START_RACE", target: race.id, details: `${race.track} • ${race.date} ${race.time} → Ongoing` });
    setConfirmStarting(null);
    toast.success("Race đã bắt đầu", { description: `${race.id} chuyển sang Ongoing` });
  };

  const [transitioning, setTransitioning] = useState<Race | null>(null);
  const [pendingState, setPendingState] = useState<RaceState | null>(null);

  const STATE_LABELS: Record<RaceState, string> = {
    draft: "Draft", open: "Open", closed_registration: "Closed Registration",
    ongoing: "Ongoing", finished: "Finished", published: "Published",
  };

  const doTransition = (race: Race, toState: RaceState) => {
    const updated = { ...race, raceState: toState };
    setRows(r => r.map(x => x.id === race.id ? updated : x));
    auditLog.add({ actor: "System Admin", action: "TRANSITION_RACE", target: race.id, details: `${race.raceState ?? "draft"} → ${toState}` });
    setTransitioning(null); setPendingState(null);
    toast.success("Race state updated", { description: `${race.id}: ${STATE_LABELS[race.raceState as RaceState ?? "draft"] ?? "?"} → ${STATE_LABELS[toState]}` });
  };

  const getNextStates = (race: Race): RaceState[] => {
    const current = (race.raceState ?? "draft") as RaceState;
    const all: RaceState[] = ["draft","open","closed_registration","ongoing","finished","published"];
    return all.filter(s => canTransitionRace(current, s));
  };
  ...
  return (
    <div>
        ...
        columns={[
          ...
          { key: "actions", header: "Actions", render: r => (
            <div className="flex gap-2">
              {r.status === "Scheduled" && (
                <Button variant="primary" onClick={() => setConfirmStarting(r)}>
                  <Play className="h-4 w-4" /> Start Race
                </Button>
              )}
              {getNextStates(r).length > 0 && (
                <div className="relative group">
                  <Button variant="secondary" onClick={() => setTransitioning(r)}>
                    Change State ▾
                  </Button>
                </div>
              )}
              <Link to="/admin/race-control/$raceId" params={{ raceId: r.id }}><Button variant={r.status === "Scheduled" ? "secondary" : "primary"}>Race Control</Button></Link>
      ...
      {transitioning && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => { setTransitioning(null); setPendingState(null); }}>
          <div className="bg-card rounded-lg shadow-xl w-full max-w-sm" onClick={e => e.stopPropagation()}>
            <div className="px-6 py-4 border-b border-border">
              <h3 className="text-lg font-semibold text-foreground">Change Race State</h3>
              <p className="text-xs text-muted-foreground mt-1">{transitioning.id} — current: <b>{STATE_LABELS[(transitioning.raceState ?? "draft") as RaceState] ?? transitioning.raceState ?? "draft"}</b></p>
            </div>
            <div className="px-6 py-4 space-y-2">
              {getNextStates(transitioning).map(s => (
                <button
                  key={s}
                  onClick={() => setPendingState(s)}
                  className={`w-full text-left px-4 py-2.5 rounded-md border text-sm transition-colors ${pendingState === s ? "border-primary bg-primary/10 text-primary font-semibold" : "border-border bg-muted/30 text-foreground hover:bg-muted"}`}
                >
                  → {STATE_LABELS[s]}
                </button>
              ))}
            ...
            <div className="px-6 py-4 border-t border-border flex justify-end gap-2">
              <Button variant="secondary" onClick={() => { setTransitioning(null); setPendingState(null); }}>Cancel</Button>
              <Button variant="primary" disabled={!pendingState} onClick={() => pendingState && doTransition(transitioning, pendingState)}>Confirm</Button>
            </div>
          </div>
        </div>
      )}
